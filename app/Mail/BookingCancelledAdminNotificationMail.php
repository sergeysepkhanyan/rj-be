...
class BookingCancelledAdminNotificationMail extends Mailable implements ShouldQueue
{
    ...
    public function build(): self
    {
        ...
        $payload['cancelReason'] = $this->reason ?? $booking->cancel_reason ?? 'No reason provided';
        $payload['cancelledByName'] = $booking->cancelledBy?->name ?? 'Customer';

        $reference = $payload['reference'] ?? ('#' . ($payload['id'] ?? $booking->id));
        $subject = 'Booking Cancelled ' . $reference . ' - ' . ($payload['date'] ?? '');
