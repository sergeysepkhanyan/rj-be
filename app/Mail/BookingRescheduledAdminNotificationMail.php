...
class BookingRescheduledAdminNotificationMail extends Mailable implements ShouldQueue
{
    ...
    public function build(): self
    {
        ...
        $subject = 'Booking Rescheduled ' . $reference . ' - Now: ' . ($payload['date'] ?? '') . ' ' . ($payload['startTime'] ?? '');

        return $this->subject($subject)
            ->from(config('mail.from.address'), config('mail.from.name'))
            ->view('emails.booking-rescheduled-admin')
            ->with(['b' => $payload]);
    }
}
