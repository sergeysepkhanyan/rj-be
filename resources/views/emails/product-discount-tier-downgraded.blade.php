...
<head>
    ...
    <title>Store Discount Updated</title>
</head>
            ...
            <table role="presentation" width="640" cellpadding="0" cellspacing="0" style="background:#fff; border-radius:14px; overflow:hidden; box-shadow:0 8px 30px rgba(0,0,0,0.06);">
                <tr>
                    <td style="padding:22px 24px; background:#111; color:#fff;">
                        ...
                        <div style="font-size:13px; opacity:0.9; margin-top:10px;">Store Discount Updated</div>
                    </td>
                </tr>
                <tr>
                    <td style="padding:32px 24px;">
                        <div style="font-size:16px; margin-bottom:8px;">Hi {{ $userName }} 👋</div>
                        <div style="font-size:15px; line-height:1.6; color:#333; margin:16px 0;">
                            @if($newDiscount)
                                Your store discount has changed from <strong>{{ $previousDiscount }}%</strong> to <strong>{{ $newDiscount }}%</strong>.
                            @else
                                Your <strong>{{ $previousDiscount }}%</strong> store discount is no longer active.
                            @endif
                        </div>
                        <div style="font-size:14px; line-height:1.6; color:#666; margin-bottom:24px;">
                            This can happen when a recent order was refunded or cancelled and your qualifying spend changed. Keep shopping to reach the next tier again.
                        </div>
